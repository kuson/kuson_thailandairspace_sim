...
## 2026-06-12 (c) — Betterment-9: INTERCEPT combat mode · STATUS: PHASES COMPLETE

**Session:** B9 full implementation, 8 tasks T1–T8 per `20260612_ShebangPlaybook.md §B9`. Orchestrator (Fable 5) + Sonnet 4.6 executor pattern; three checkpoints browser-verified: C1 post-T3 (weapons), C2 post-T6 (INTERCEPT wave), C3 post-T7 (shadow blobs + regression). Branch `betterment7-20260612` (B9 commits follow directly from B8 HEAD `20946e2`). Verification harness: rAF fully suspended this session — even screenshots did not wake it; ALL verification driven manually through `window.__sim` handles + manual `renderer.render` frames. Ground-tile updates unreachable manually because bootstrap overwrites `window.__sim.ground` with `{rangeRings, airports, provinces}` — pre-existing collision, same family as the `tourGuide` note in journal (a).

**T1 — Trail system (`3f25b79`):** preallocated 120-pt `Float32Array` trail buffers, in-place writes + `setDrawRange`, `frustumCulled = false`; trim loop caps `n ≤ 120` before every rebuild. **Baseline (recorded in commit body):** ground (200 m) 346 draw calls / 19 218 tris; 40 000 ft 332 calls / 17 484 tris; airspace groups hidden at 40 k ft → 77 calls (airspace ≈ 255 calls); threshold ~450 NOT exceeded → per-group culling SKIPPED per measured go/no-go.

**T2 — Aim system (`a597bef`):** FOV eases 70 → 58° in ≤ 150 ms when `drone.locked`; rate computed per-instance from `origFov` (orchestrator review caught initial bug: fixed 6.67°/s vs required 80°/s — fixed pre-merge). Fire gated to locked-only; `F` key autofire verified unbound; listener-leak-safe enable/disable.

**T3 — Weapons hit math (`cef1eab`):** `raySphereT` analytical quadratic (unit-dir; `t1 ≤ 0` behind-camera reject; inside-sphere exit-point `t2`); nearest-first within 4 000 m. Orchestrator review fixes pre-merge: per-shot `Vector3` allocation removed (module scratch); shielded "small flash" implemented as 4-spark blue-white burst; spark colour = per-slot ember tint (was red-only); missing `play("fire")` per-shot tone added. **C1 browser evidence:** on-ray hit / lateral-miss (offset 80 > r60) / nearest-first (800 vs 1 500 m) / beyond-range (4 500 m) all correct; heat locked at shot 13 exactly (8 × 13 = 104 → cap 100), overheat tone, blocked while locked, recovered after update (1.6 s); 10 shots cycle 8-tracer pool, all fade; aim FOV 70 → 58 → 70 with reticle show/hide; fire gating locked-only (unlocked click = 0 emissions); dispose removes tracer group + heat bar. Pixel-baseline A/B vs pre-B9 (detached checkout `7a4acdc`, same manual-frame procedure): identical scene, 390 calls / 20 584 tris both builds; purple/tan harness-visual confirmed NOT a regression (untextured tiles, VTR1 walls from inside).

**T4 — UFO combat AI (`30c20ca`):** combat opts `{combat, shielded, orbitRadius}`; SCRAMBLE spawn byte-identical (no `hp`, no shield, no mat clones, glow 0.555 normal pulse). Review fixes pre-merge: EVADE speed was radius-coupled (8 km orbit → ≈1.7 km/s jinks!) → now behaviour-based (`ATTACK_RUN` 105 m/s, orbital capped 120 × 1.5 = 180 m/s); ORBIT-resume angle reseed. Browser: shield glow ×1.6 (0.814 = 0.509 pulse × 1.6); shielded hit no-op; unshielded hit → hp 2 + white emissive flash + `EVADE`; reach-target fired once at 398 m → auto-`RETREAT`; 3rd hit → explode tone + `onExplode` + geometry count returned to 249 baseline (banish AND destroy paths).

**T5 — Crawler layer (`de1592c`):** 6 crawlers ring-spawned at exactly 10 000 m, converge exactly 15 m/s (9 100 m after 60 s), terrain-follow `y = elevationAt(lat,lon) + 12` sampled ≤ 2 Hz (y = 18 at Bangkok), reach-base fired once at ≤ 500 m + despawn, hp 2 destroy → explode + dispose; shared dome/ring geometry + materials module-level (clone-on-destroy-fade for opacity); radar blips via `ui.setGameBlipProvider` — red pixel count 195 → 244 with provider, → 195 after unregister.

**T6 — INTERCEPT mode wiring (`22c270a`):** mode row SCRAMBLE | INTERCEPT (ArrowLeft / ArrowRight on briefing card), `kuson.game.v1.mode` persisted; defended airspace = random CTR with matching `airports.json` icao (e.g. VTUU-CTR Ubon, VTSS-CTR Songkhla picked in tests); raiders = `tier.contacts + waveIndex − 1` shielded UFOs ORBIT 8 km + `2 + waveIndex` crawlers 10 km ring. Review fixes pre-merge: `atc.call` positional → object args; `aim.offFire` added (per-wave callback leak). **C2 evidence:** WIN — gate solve dropped all 3 shields + `lockSweep` + "Weapons free, weapons free."; `ATTACK_RUN` staggered exactly 0/5/10 s; score exact: (3×150 + 3×75)×1 + accuracy 100 + integrity 100 = 875; debrief card `"WAVE 1 — CADET — WON | BANDITS DOWN | UFO KILLS 3 · CRAWLERS 3 | ACCURACY 100% | INTEGRITY 100 | WAVE SCORE: 875 | SESSION TOTAL: 875 | BEST: 875"`; persisted `bestScore`/`waveReached`/`mode`. LOSS — wave 2 (4 UFOs + 4 crawlers): 3 UFO reaches (−60) + 4 crawler reaches (−40) → integrity 0 at 634 sim-seconds → "WAVE 2 — CADET — BASE OVERRUN"; accuracy 0 with 0 shots (no `NaN`). **Fix found at C2:** UFO spawned at world origin until first orbit tick — gate solved within spawn frame sent a raider on a 715 km return flight from Bangkok to VTSS (`b36850a` places holder on orbit at spawn; browser re-verified: all raiders exactly 8 000 m from centroid at spawn). **Second fix:** destroyed UFOs stayed targetable during 0.4 s death fade — corpse soaked 9 extra hits inflating accuracy (`b89f081` filters `DESTROY`/`BANISH` from `getCombatEntities`; crawlers already filtered). Real in-range hitscan kill verified post-fix (12/12 hits at 2 km, kill counted, 64-vert spark pool saturates + wraps). Abort mid-fight (incl. with 8 UFOs + 6 crawlers live): IDLE, 0/0 entities, reticle + heat bar hidden, FOV 70, blip provider unregistered, typing closed, weapons provider stubbed, strip hidden. SCRAMBLE regression: ArrowLeft → scramble persisted; non-combat spawns (no hp/shield); reticle/heatBar NEVER DOM-created in a scramble-only session; contact auto-challenge in-volume (note: `lowerFt`/`upperFt` ARE converted ft → m via `FT_TO_M` in `airspacesAt` — mid-band of 8 000 ft ceiling = 1 219 m); solve → banish tone + 200 pts; abort → IDLE 0 UFOs; `bestScore` 875 preserved across mode switch.

**T7 — Shadow blobs (`b14f160`):** ONE shared 128² radial `CanvasTexture` + ONE unit `PlaneGeometry` rotated flat (NOT `THREE.Sprite` — billboarding tilts ground shadows); per-blob cloned material (independent opacity); `k = 1 − clamp((agl − 200) / 1800, 0, 1)`; scale = `base × (0.5 + 0.5k)`; opacity = `0.35k`; player blob (base 8 m) always-on in main loop, ≤ 2 Hz terrain cache; combat UFOs (60 m) + crawlers (35 m, `groundY − GROUND_OFFSET` corrected) per-entity; non-combat UFO path untouched. Orchestrator amendments: UFO `shadowGroundY` initialised to force first-update sample (was `baseY` → blob floated at flight altitude 0.5 s); `makeShadowBlob` starts hidden until first update (was visible unit-blob at origin). **C3 evidence:** 15 blobs during raid (1 player + 8 UFO + 6 crawler), 14 visible at terrain heights 162–175 m (player blob updater lives in suspended rAF loop — harness limitation; law verified via the 14 entity blobs on the same code path). Pre/post-B9 parity 390 calls / 20 584 tris; perf full-update-chain idle 1.69 ms/frame vs 7.89 ms/frame with 8 UFOs + 6 crawlers + firing + combat AI (CPU proxy, rAF suspended; ~6 ms combat cost, far under 16.6 ms budget; calls 273 in-fight vs 335 idle — no draw-call growth); B5 danger toggle −130 calls (335 → 205 → 335 exact restore); volume-glow OFF pixel-identical (A/B verified); tutorial start/abort clean; safety alerts still preempt (NO-FLY VTD65 banner fired during testing); console errors pinned at 96 all session — all 96 from early orchestrator harness probing on PRE-B9 code (stale-tile NaN `boundingSphere`), zero attributable to B9 commits.

**T8 — Docs:** spec §3.16 INTERCEPT subsection + §6 acceptance rows 45–52; this journal block; `state_TODO.md §0c`.

**Commits (in order):** `3f25b79` trails T1 · `a597bef` aim T2 · `cef1eab` weapons T3 · `30c20ca` UFO combat T4 · `de1592c` crawlers T5 · `22c270a` INTERCEPT T6 · `b36850a` fix UFO spawn position · `b89f081` fix dying-UFO targeting · `b14f160` shadow blobs T7 · this docs commit (T8).

**Known / deferred:**
- Shadow blobs for live-traffic aircraft — deferred, cost.
- Wave-1 CADET INTERCEPT is mathematically unlosable (3×20 + 3×10 = 90 < 100 integrity) — acceptable difficulty ramp, noted only.
- Player shadow blob not verifiable in suspended-rAF harness — code-reviewed; verify visually in a live session.
- `__sim.ground` bootstrap overwrite hides `DynamicGround` (pre-existing; blocks manual ground updates in harness).

**End HEAD:** `b14f160` (pre-docs commit; this docs commit — spec INTERCEPT + journal (c) + TODO §0c — follows immediately on the same branch).

---

## 2026-06-12 (d) — Betterment-10: World Beauty · STATUS: PHASE COMPLETE

**Session:** B10 full implementation, 9 tasks T1–T9 per `20260612_ShebangPlaybook.md §B10`. Orchestrator (Fable 5) + Sonnet 4.6 executors (T1, T3, T4, T5, T7, T8, spec half of T9); T2 + T6 implemented by Fable inline per the §B10 routing decision. Two executor mid-task deaths (T3 after the 4 layer files, T8 after physics.js) — remainder finished inline per §B10.0 precedent. Checkpoints C1 (post-T3), C2 (post-T6), C3 (post-T8) browser-verified. Branch `betterment7-20260612` from B9 HEAD `41c96c3`.

**Harness this session (supersedes the B9 note):** rAF is VISIBILITY-GATED, not dead — it ticks in bursts around `preview_screenshot` calls (frame-counter delta 0/s between them). That explains two reproducible scene "attractor states": pre-label-cull 514 calls / 25 908 tris and post-cull 360 / 20 272 (the cull pass runs on the first loop tick). Async loads (tiles, terrain.bin, airports.json) STALL while the tab is backgrounded and the canvas collapses to 0×0/1×1 between screenshots — sequence every probe as screenshot-wake → settle → eval, and compare A/B numbers only within the same attractor state. Headless GL is SwiftShader (see T5).

**Pre-B10 baseline (detached `41c96c3`, §B10.0 procedure):** early state 514 calls / 25 908 tris / 330 geo / 127 tex / 14 progs; settled 360 / 20 272 / 15 progs; manual-frame 2.91–2.96 ms.

**T1 — __sim handles (`d7b59aa`):** bootstrap bag renamed `window.__sim.groundLayers`; `__sim.ground` = DynamicGround (updateAround callable in browser ✓); `__sim.tourGuide` defined post-bootstrap ✓. Grep: the old bag was write-only — no consumer broke.

**T2 — Terrain displacement (`9c027a2`, Fable inline):** per-vertex CPU displacement at tile build from `elevationAt` (the same bilinear grid as AGL/crawlers — visuals and physics agree by construction); SEGS 24/24; `aSea` attribute (elev ≤ 0.5 m) written in the same loop; `onTerrainReady()` rebuild-once; toggle OFF ⇒ 1×1 geometry byte-identical (4 verts, no aSea, tris 25 908 exact). Browser: Bangkok tile 2.3–8.4 m; Doi Inthanon tile 591–2 533 m (summit 2 565); 74 tiles; terrain ON at baseline view 81 108 tris (+55 200 rendered, under the ~85 k worst-case budget).

**T3 — Objects on relief + drone clamp (`1b4a813`):** city/airport beacons lift `elevationAt + offset` with `reliftToTerrain()` called from the terrain-load `.then` (install happens before terrain.bin resolves — orchestrator-flagged race); range rings ride drone ground elevation + 0.5 (≤2 Hz crawler pattern; group lift, labels too); drone floor `elevationAt + 1.5` ALL modes (≤2 Hz cache), `resampleTerrainFloor()` on teleport AND flyTo completion (physicsStep is locked during flyTo — without the arrival clamp a mountain warp sits underground up to 0.5 s). Executor died mid-task; orchestrator finished main.js wiring, rangeRings `dt` call-site, the resample extraction + flyTo clamp.

**CHECKPOINT C1 (PASS):** Doi Inthanon massif relief visible, no seam cracks — programmatic shared-edge max gap **0.0004 m** across 20 adjacent detail pairs; Bangkok flatlands visually unchanged; terrain OFF → A/B parity with pre-B10 EXACT in both attractor states (514/25 908 and 360/20 272; geo/tex match); frame-ms ON 2.39–3.28 vs pre-B10 2.91–2.96 → **delta ≈ 0, +2 ms gate passes, SEGS_BASE stays 24**; clamp: teleport y=500 at summit → arrival-clamped 2 496.9 (= 2 495.4 + 1.5), forced y=1000 → one physicsStep → 2 496.9, AGL exactly 1.5 m steady; VTCC airport marker at elev 309 + 120 ✓; rings group y 2 495.9 at the summit ✓. Province lines stay flat per contract — they bury under northern relief → TODO row filed.

**T4 — Day/dusk/night (`6e79397`):** `installDayNight({scene, skyRig, hemi, sun, groundTiles})`; ramp table §B10.0; DAY short-circuit writes exact constants then skips (byte parity); auto = Asia/Bangkok wall clock; tile tint only on textured materials (placeholder guard). Orchestrator review fixes pre-merge: TDZ crash (module-level install referenced `ground` before its `const` — page wouldn't boot; node --check can't catch TDZ) and `getNightFactor` returning the night-band weight (dusk read 0 — now night + dusk×0.6 → **0 / 0.6 / 1** exact). Sky dome driven via Preetham params (rayleigh/turbidity/mie + sunPosition) — sky.js exposes no horizon/top color uniforms, so the table's night colors are approximated (dark teal-grey vs navy; dusk reads convincingly warm). Fog exact: a6cdee / d9a07a / 0a1020.

**T5 — Night city + airport lights (`77a4154` + fix `03bd1b7`):** ONE Points / ONE CanvasTexture / additive ShaderMaterial, per-vertex world-metre `aSize` (cities 600/900/1300 by prominence, airports 700), `elevationAt + 30` with relift, async airport fill into a pre-sized buffer via drawRange (no rebuild on mode flips — counts verified stable). Orchestrator fixes: executor's pixel-space size math (aSize × 300/−z ≈ sub-pixel at cruise) → world-metres × uScale(viewport, fov)/depth with 64 px cap + zero-viewport guard; `frustumCulled = false` (B9 trail precedent; partially-filled buffer makes the auto boundingSphere unreliable); `__sim.cityLights` handle; **logdepthbuf chunks** — the renderer runs `logarithmicDepthBuffer`, every built-in material writes log depth, and a custom ShaderMaterial without the chunks ALWAYS fails the depth test (points drew but emitted nothing; root-caused by bare-scene + single-point bisection). Post-fix: bench point 3 005 px warm amber [252,171,97]; the full layer renders 2 669 px in a bare scene; full-scene on/off diff still reads 0 on THIS harness — stock-material points on the same geometry ALSO vanish in the full scene on the same load, while both render standalone → SwiftShader state-dependent point-raster artifact, not app code. **Live-hardware visual check filed as TODO.** Opacity law verified: 0 / 0.54 / 0.9 at day/dusk/night, visible gate > 0.05, toggle OFF removes.

**T6 — Water shimmer (`97c5a1f`, Fable inline):** `onBeforeCompile` on the shared tile material (B8.T9 pattern); sea fragments (aSea) mix toward deep blue + two crossing sine bands (combined amplitude ≤ 0.05; periods ~80 m / ~210 m) scrolled by ONE module uTime ticked in the loop; constant `customProgramCacheKey` ⇒ one extra program per map-state; `uWaterOn` uniform gate. Browser (Gulf 13.0 N 100.6 E): nearest detail tile 94 % sea verts; uTime scroll changes 8 083 px; OFF → 0 px across time; inland ON/OFF → 0 px (land untouched); programs 15 → 16.

**CHECKPOINT C2 (PASS):** mode select flips world day↔dusk↔night with persistence (`kuson.daynight.v1`, survives reload + select restores); DAY default parity exact (terrain OFF 360 / 20 272; fog/tint/light constants byte-equal); dusk screenshot warm sun horizon + haze band; night screenshot dark navy world; city-light law + counts as above (visual deferred to live hardware); water as above; program delta vs pre-B10 ≤ +2 (water +1, night-lights +1); geometry/texture counts stable across flips (only background streaming deltas); console clean.

**T7 — Wind + crosswind HUD (`0345578`):** altitude profile knots 500/3k/6k/11k → ×1.0/×1.5/×2.0/×2.8 linear-clamped; veer 15°/3 km CW above a 500 m dead-band (strict surface parity — REPL 3/3 samples bit-identical to pre-B10 at y=0); veer applied before vec3 derivation (physics drift and chip agree). REPL: multipliers/veer exact at all four altitudes. Browser: surface 5.7 m/s @222° → 6 km 11.4 m/s @252° = ratio **2.00**, veer **+30°** exact; chip `"222° / 5.7 m/s · X 3.8L T 4.2"` → 180° heading flip → `"X 3.8R H 4.2"` (L/R and T/H both flip ✓).

**T8 — G-limits (`c09b44a` + handle `c4d02f7`):** `n = 1/cos(bank) + q·V/g` published as `drone._loadFactor` per fixed-wing step (q from Δpitch/dt with dt guard; **drag keeps the pre-B10 turn-only term** — orchestrator review fix so instrumentation doesn't alter aero); limits on FW_PRESETS (C172 +3.8/−1.5, Learjet +4.4/−1.8, B777 +2.5/−1.0 — read back exact); buffet reuses `_buffetT` with proximity-scaled accumulation; one-shot `gLimit` tone (880→660) with 0.85 hysteresis; ADVISORY after >1.5 s cumulative exceedance, auto-retract; G row beside WIND (minimal-HUD hidden); watcher in main.js (alerts+audio seam, `droneStateForAudio` precedent). Executor died after physics.js; orchestrator finished inline. REPL: `loadFactor === 1/cos(roll)` float-exact at q=0 (= 2.0 at 60°; note ROLL_MAX is 45° so 60° is formula-level, not flyable); pull-up +9 / push-over −7 signs ✓. Integration (via `__sim.gLimitWatch`): tone fires entering the 0.9 band, no re-fire in-band, re-arms below 0.85; alerts snapshot during exceedance `glimit: AIRFRAME OVERSTRESS — n=4.6`, retracted after recovery; easy mode G 1.0 inert.

**CHECKPOINT C3 (PASS):** B5 danger toggle 309 → 184 → 309 exact restore; B6 airports −13 / provinces −6 calls, rings +6, all exact restores; B7 SCRAMBLE IDLE→BRIEFING (card visible)→WAVE→abort→IDLE, mode persists; B9 INTERCEPT wave spawns exactly 3 shielded UFOs + 3 crawlers (cadet wave 1), reticle present, abort → IDLE 0/0 entities, FOV 70, mode persists; B8 audio tick plays, HUD minimal toggles both ways, tutorial start/abort clean (card render needs a live loop — B10 touched none of its paths); all four new toggles OFF→parity re-confirmed; final manual-frame 1.94–2.01 ms (vs C1 baseline 2.91–2.96 — no regression); **console clean the ENTIRE session: zero errors, zero warnings across every reload, game smoke, and shader compile.**

**T9 — Docs:** spec §3.18 "World (Betterment-10)" + §6 acceptance rows 53–60 (Sonnet executor); this journal block + `state_TODO.md §0d` (orchestrator); push.

**Commits (in order):** `d7b59aa` T1 handles · `9c027a2` T2 terrain · `1b4a813` T3 objects+clamp · `6e79397` T4 daynight · `77a4154` T5 city lights · `97c5a1f` T6 water · `03bd1b7` fix city-lights logdepth · `0345578` T7 wind · `c09b44a` T8 G-limits · `c4d02f7` debug handle · this docs commit (T9).

**Known / deferred:**
- Province LINES stay flat → buried under terrain in mountainous regions (invisible there). Lifting long polylines onto relief is a heavier task — deferred TODO row.
- City lights: every controllable instrument passes (bare-scene render, opacity law, counts); the composed-scene visual could not be confirmed under SwiftShader (state-dependent point-raster artifact also affecting stock PointsMaterial). **Verify visually in a live hardware session.**
- flyTo/tour lerp can pass THROUGH a mountain mid-flight (cinematic path; only arrival is clamped) — pre-existing path family, newly visible with relief. Noted, not in B10 scope.
- Night/dusk sky-dome colors are a Preetham-parameter approximation of the §B10.0 table values (sky.js exposes no direct color uniforms).
...
## 2026-07-02/03 (h) — Betterment-11 "Facelift" — EXECUTED · branch `betterment11-20260702` · GATES C1–C4 PASSED

**Operator:** Fable 5 orchestrator (autonomous, per `20260702_FaceliftPlaybook.md`); Sonnet executors T1–T10/T12–T14 + spec; T11 + fixes orchestrator-inline. Real-hardware Chrome (M1 Metal) via extension MCP + Bash dev server; pre-B11 worktree on :8082 as the parity reference.

**T0 (git ops):** `main` fast-forwarded 71c38dc→cab0722 (ancestor check held) and pushed; branch `betterment11-20260702` created+pushed. Baselines: 6 canvas PNGs + notes in `.scratch/facelift_20260702/` (renderer half-res anomaly at capture explained later: per-origin Chrome zoom).

**Shipped (16 code commits):**
- **T1 `d2fa09a` uiPrefs registry** — 20 ids, sparse `kuson.uiprefs.v1`, J/H/backquote rewired (T/C keys didn't exist — added in T2); `attitude` default corrected to false (parity beats the design table); hudExtras externalStore bridges `kuson.hud.v1`.
- **T2 `b6c24f0` View panel + ⚙ radar popover** — 20 rows w/ key hints, mode-tab stub, reset; floaters folded (radarOptions default→false per contract); HUD buttons + pro-chip routed through the registry.
- **T3 `3aef79f`+`e8f2634` themes** — classic/daylight/nvg/amber; CSS `[data-theme]` blocks own vars (no-flash sync head script) + `theme.js` owns canvas/branch palettes; 53 accent-alpha rule-literals → `color-mix(var(--accent))` (computed-style byte-equal in classic); danger≥4.5:1 all themes.
- **T4 `52b1c89` panel IA** (design-reviewed pre-commit) — TOUR/VIEW/WORLD/TRAFFIC/FLIGHT/INFO collapsible groups, `kuson.panel.v1`, fresh default TOUR+WORLD open; disclaimer → fixed footer, 245 chars byte-identical, wraps-never-truncates (executor caught the 1280px ellipsis trap).
- **C1 fix `02e4019`** — compass draw-guard kept the old palette after theme swap while stationary; onThemeChange → `_hudCache={}` (house repaint idiom). **C1 PASS**: defaults-parity 0/921,600 px vs pre-B11 worktree (355 calls/22,112 tris/14 progs equal), theme cycle probes 92/92/95/98 distinct, persistence pass.
- **T5 `9f08c27` appMode** — freestyle/learning/game+detail from FSM/tour/tutorial notifications; guards with 3s ADVISORY chips ("End the tour/mission first"); live walk emitted the exact expected sequence.
- **T6 `21a5fc6` uiProfiles** — session layer in uiPrefs (mid-mode toggles ephemeral); wave profile hides panel/chips/history/identify/radarOptions/controlsHint/simSpeed/districtLabel + shows radioLog; learning formalizes the tour label flip (tourGuide `_savedLabels` removed); per-mode overrides beat profiles; exact-restore proven by snapshot equality.
- **T7 `14f3ac8`+`fb3aae1`+`98c253a` input scoping + Escape ladder** — `inputAllowed` matrix (79-cell Node-verified); V reclassified viewToggles (§0.5.4 shorthand was wrong); C2 found+fixed a ladder deadlock (card-node truthiness) and missing overlay CSS. **C2 PASS**: WAVE storm state-frozen (warp chip inert), Esc open→dismiss→reopen→confirm round-trips both cards, briefing Esc closes.
- **T8 `4a75653` interior-fill fade** — clone-on-need (explicit onBeforeCompile/customProgramCacheKey copy — Material.copy drops them), walls+floors one geometry, 0.15× target clamped ≥0.03 (measured exactly 0.03), setHighlighted stomp fixed; program-delta 0.
- **T9 `6018486` declutter laws** — executor died at session limit; work complete on disk, orchestrator reviewed (owner-pass ordering + visible-recovery + generic persistence) and committed. Ladder verified at 500 m/3 km/8 km/8.5 km; tick 0.043 ms.
- **T10 `f88543e` focus mode** — 143 non-kept volumes dim (walls 0.05/outlines 0.3), exact clone restore, F/View-row round-trip, auto-exit on mode change. **C3 PASS** incl. defaults-off parity hash `5733134d` bit-identical vs pre-B11.
- **T11 `73733ba` gated bloom (Fable-inline)** — playbook's linear chain measurably breaks translucent-fill compositing (47% px day diff; opaque terrain byte-equal — blend-space semantics); shipped a bloom OVERLAY on the exact legacy frame (framebuffer copy → TexturePass → UnrealBloom off-screen → raw CopyShader; the copy pass is load-bearing, r170 re-grades built-in-material screen blits). Night-gated after measuring day-sky luminance (0.94) > city lights (0.80): strength×nightFactor, skip below 0.05. Day 0-px diff @1.65 ms; three/addons imported from the pinned CDN tag (repo was never vendored — report §1.5 stale).
- **T12 `f32829c` night pack** — `scripts/gen_city_lights.py` (seed 20260702, double-run byte-identical) → 3,160 city + 14 airport points, one Points w/ aSize/aColor + 24 px cap; `nightSky.js` 1,200-star dome + moon opposite sun; all logdepth-chunked, nightFactor-gated (day byte-identical); `data/cities.json` generated from `THAI_CITIES` (didn't exist as a file).
- **T13 `00d6786` terrain hillshade** — elevation-grid central differences (1/120° = the DEM cell), shade clamp(0.75+0.4·dot(n,SUN_NW)); vertex colors only when ON (OFF = byte-identical build, verified 0 color attributes); toggle rebuilds live (~32 ms all 75 tiles); flat-land shade 1.0497 (executor corrected the un-normalized dot in the note); anisotropy already uniform (audit premise stale — untouched).
- **T14 `340fbd0` bug sweep** — (a) placeLabel >5 km jump invalidation at the structural choke; (b) daynight onModeChange + getMode + eager first update (reload desync = init-order gap, not lerp); (c) footer reserve arithmetic verified only; (d) alert stack measures the live TARGET strip; (e) placeLabel 52→72 px.
- **C4 fix `9880eef`** — T12's ~75× point multiplication additive-stacked distant clusters to blown white; bloom .62 also caught fresnel rims → horizon flares. Vertex-shader spread compensation (alpha 1→0.06 over 6→70 km) + retune (.80/.40). Post-fix blown px 17, warm carpet 4,501 px.

**C4 GATE (PASS):** §0.4 repros all pass · day 1.65 ms / night-everything-ON **3.77 ms ≤ 4 ms** (M1) · defaults-OFF (all five new toggles) parity **bit-identical hash `5733134d`** vs pre-B11, equal to C3's hash · B5 danger 374→230→374 exact · B6 airports exact · B7 SCRAMBLE + B9 INTERCEPT (3 crawlers, FOV 70) cycles clean · tour↔appMode round-trip · day↔night↔day exact · console zero B11-attributable errors.

**Methodology notes (for future sessions):** parity via pre-B11 git worktree on a second port + cleared profiles + exact pose copy + forced 1280×720 buffers + in-page PNG-dataURL FNV hash (sink XHR gets throttled in long-backgrounded tabs); renderer.info under the composer reports the final copy quad — read instrument counts on the legacy path; synthetic keydowns must dispatch on document.body (window-dispatch misses document-level capture listeners); start-screen dismissal must be verified before keyboard tests; per-origin Chrome zoom changes the drawing-buffer size (the T0 half-res "anomaly").
...
## 2026-08-28 (k) — Betterment-12: Headless verification rig + boot-window fixes

**Operator:** Claude Code (Fable 5 orchestrator; concurrent spec/fix subagents on one shared working tree; adversarial verifier pass)
**Start:** 2026-08-28, Asia/Bangkok
**Objective:** Stand up a zero-network headless-browser e2e rig that automates the smoke checklist (`state_TODO.md §2` / spec §6 subset) — including the Traffic-heat error path that had been pending as a manual next_action since journal (j) — and fix the two boot-window console-error sources it exposed.

### Findings
- **The rig** (`test/e2e/`): plain `node --test` specs drive the **real app** in headless Chromium (SwiftShader WebGL) via the Playwright **library** — not `@playwright/test` — so the repo stays package-json-free (claude.md §3/§4). `test/e2e/harness.mjs` starts a `node:http` static server on an **ephemeral port**, registers a catch-all abort for every external request, serves `https://unpkg.com/three@0.170.0/…` from a vendored copy of the npm `three-0.170.0` tarball (downloaded once from registry.npmjs.org into gitignored `test/e2e/.cache/`, atomic temp-dir+rename so concurrent runs don't race), and stubs carto basemap tiles with a tiny PNG. Specs: `boot.test.mjs` (5 subtests), `core.test.mjs` (9), `features.test.mjs` (9), `boot-clean.test.mjs` (1), `teleport-nan.test.mjs` (1), `debug-flag.test.mjs` (2). Run: `node --test test/e2e/*.test.mjs`. Full usage docs (incl. `PLAYWRIGHT_LIB` resolution on other machines) in `test/e2e/README.md`.
- **Traffic-heat ERROR path is now automated** (STATUS.yaml's pending manual next_action after journal (j)): `features.test.mjs` poisons `IDBDatabase.prototype.transaction` — the exact seam `src/pathHeatmap/store.js` `idbRunTx` calls — then pushes one valid position through the module's real `liveFlights.onPositions` hook and asserts **atomically** that `#heatStatus` shows the error, the `#optHeatRecord` checkbox unchecks itself, and persisted `recordingOn === false`. Deleting `window.indexedDB` pre-boot would instead select the memory-store fallback (recording keeps working) and never reach the recording-off error seam — that is why the poison targets the prototype.
- **Declutter boot bug (fixed):** `src/main.js` declared `declutterLayers` as a `const` *inside* async `bootstrap()` while module-scope `loop()` referenced it; `startSim()` starts rAF before bootstrap resolves, and a function-scoped const is never visible to a sibling module function — so `_safe("declutter", …)` threw `ReferenceError: declutterLayers is not defined` **every frame** (console spam during boot, and the declutter pass in fact never ran at all, even post-init). An audit of every identifier `loop()` touches confirmed this was the only bootstrap-scoped leak.
- **Teleport-NaN root cause (fixed — closes the §3 `computeBoundingSphere` TODO):** a large/staged teleport past the Web-Mercator latitude domain (|lat| > 90°, e.g. ~9,000 km north) makes `latToTileY()` (`src/coords.js`) return NaN via `Math.log` of a negative; `DynamicGround.updateAround()` fed those NaN indices to `_buildMesh` → `tileYToLat(NaN)` → `new THREE.PlaneGeometry(width, NaN, …)` → THREE's `computeBoundingSphere(): Computed radius is NaN` on every rendered frame. Once `data/terrain.bin` streamed in, `elevationAt(NaN, NaN)` slipped past the old `fy < 0 || …` range guard (false for NaN) and baked NaN vertex heights on top.
- Residual, **verified benign**: `src/ui.js` `_drawMapUnderlay` (~line 2834) computes NaN tile centers while parked at |lat| > 90°, but both its for-loops compare against NaN bounds and never execute — an empty minimap basemap in that out-of-domain state, no throw, no console error. Left alone.

### Changes applied
- `test/e2e/harness.mjs` + `test/e2e/README.md` — the harness (`launchSim` / `waitForSim` / `dismissStartScreen` / `pressKey`), vendored-three cache, ephemeral-port static server, abort/stub network routes.
- `test/e2e/boot.test.mjs` — boot/readiness smokes (5 subtests).
- `test/e2e/core.test.mjs` — 8-item core-interaction checklist, all automated, each subtest restoring neutral state: presets 1–5 (HUD label + `_currentModel.uuid` swap), held-`W` movement (>50 m), `P` pause freeze + button label + resume, `U` metric↔aero HUD strings, `M` map-primary (canvas resize + label prefix), `I` identify cap-3 + expand pill, `V` 1st↔3rd via `__sim` state (no DOM badge exists for view-person), HUD lat/lon-DMS/SPD/HDG sanity + update.
- `test/e2e/features.test.mjs` — catalog warp to a >100 km target (lands <1.5 km from the volume's S-side overviewVantage), 3-warp undo/redo round-trip (tolerant of mid-warp BOUNDARY entries), express tour (narration overlay, `i` blocked while `drone.flightLocked`, End restores), Strict-CAAT reload persistence (`kuson.sim.strictCaat`), Traffic-heat nominal (mock fleet → `recording`) / ERROR (IDB poison, runs last in its session) / settings-reload (`kuson.pathHeatmap.settings.v1`) paths. Two shared browser sessions.
- `src/main.js:301,694,1178` — hoisted `let declutterLayers = null` at module scope (the existing `ui`/`tourGuide` hoisted-let idiom), bootstrap assigns it, loop guards `declutterLayers?.update(camera, drone, dt)` — no-op until install lands, originally-intended declutter behavior afterward.
- `test/e2e/boot-clean.test.mjs` — boots, idles 8 s, asserts an **empty** filtered console-error list (only harness-aborted `Failed to load resource` excluded); fails by construction on pre-fix code (per-frame ReferenceError).
- `src/ground.js:403-425` — `updateAround()` now finite-guards the incoming world position **and** all four base/detail tile indices before touching any state (`_lastWorld`/`_lastBase`/`_lastDetail` stay untouched on a bad position, so the first sane position refreshes normally). Every `_ensureTile`/`_buildMesh` call flows through this seam, including the `onTerrainReady` rebuild, which replays `_lastWorld` — a value that can only ever hold already-guarded positions; `_clearTiles`' existing NaN sentinels still retrigger rebuild-after-clear (verifier-audited composition).
- `src/terrain.js:64-70` — `elevationAt` range guard rewritten in negated in-range form (`!(fy >= 0 && …)`) so NaN inputs take the return-0 out-of-coverage path; identical reject set for finite inputs. Covers all ~25 call sites repo-wide.
- `test/e2e/teleport-nan.test.mjs` — repro with terrain data **delayed** so both sides of the load gap are covered: warp out of the Mercator domain during the gap (positive control: terrain requests held, `isLoaded() === false`), let the grid arrive while far out, assert zero NaN tile keys AND zero `computeBoundingSphere` errors, then warp back and assert real elevations apply (`pos.y > 1000` at a mountain) and the frame counter still advances.
- `src/debugOverlay.js` — §3-Low frame-budget logger behind `?debug=1`: `Float64Array` ring inside the overlay's existing once-per-frame `update()` seam; one `console.info` `[frame-budget] avg … | p95 … | worst …` line per ~5 s; >1 s gaps skipped as tab-pauses; flag absent ⇒ one falsy test per frame, zero allocation, zero output.
- `test/e2e/debug-flag.test.mjs` — with `?debug=1` at least one format-matched budget line lands; without the flag, none.
- `data/airspaces.schema.json` — §3-Low JSON Schema (draft-07) for the catalog: `meta` + `volume` definitions, circle/polygon variants, lat/lon bounds.
- `test/data/airspacesSchema.test.js` — validates `data/airspaces.json` against the schema with hand-rolled checks whose required lists/enums/bounds are **read from the schema file itself** (no validator dependency, no silent drift). 9 tests.
- `.gitignore` — `test/e2e/.cache/`.

### Decisions
- Playwright **library** + preinstalled Chromium (resolved `import("playwright")` → `$PLAYWRIGHT_LIB` → container global) — because the zero-build / no-npm-install rule is non-negotiable; nothing test-related enters the app's runtime.
- Zero network by construction (catch-all abort registered first; specific fulfill routes after, which Playwright matches first) with three@0.170.0 vendored from the registry tarball — because tests must not depend on unpkg/carto/nominatim availability, and the vendored copy is byte-for-byte what the importmap pins.
- IDB failure injected at `IDBDatabase.prototype.transaction` (the `idbRunTx` seam) rather than removing `indexedDB` — the latter never exercises the recording-off error path journal (j) fixed.
- `V` asserted via `__sim` state + chase-model visibility — no DOM badge exists for view-person (`viewModeBadge` covers only down/left/right).
- Teleport-NaN guarded at the two seams (`updateAround()`, the single entry to tile geometry; `elevationAt()`, shared by every elevation consumer) rather than chasing individual geometry consumers.
- Commit SHAs: **committed after this block** — the docs closeout runs before the wave's commits, so none are quoted here. Branch: `claude/workflow-execution-vz9bof`.

### Pending verification
- [x] `core.test.mjs` subtest 3's paused-freeze assertion was wall-clock (hold `W`, sleep 350 ms, assert <1e-6 m moved): a zero-frame SwiftShader window would pass vacuously. FIXED before commit: `waitForRenderedFrames(page, 2)` between hold and read (rendering continues while paused — only physics halts — so frames prove loop iterations happened); unused `sleep` helper removed; re-run 9/9 green.
- [ ] §2 rows the rig does not cover stay manual: pointer-lock/mouse-look, `Space` hover, arrow-view badges, Reset-clears-history, HDG inertia feel, altitude-tape visuals, full-tour completion, nested CTR/TMA flicker, 3D label toggles.
- [ ] Player shadow blob foreground visual (carried from §0d/§0e — unrelated to this wave).

### Session close
- TaskList: none opened (orchestrated wave; every agent left its deliverable verified green).
- Tests at close: `node --test test/e2e/*.test.mjs` → **27/27** twice (66.0 s / 60.8 s flake probe, zero skips); `node --test test/pathHeatmap/*.test.js` → **31/31**; `node --test test/data/*.test.js` → **9/9**; `node --check` clean on `src/main.js`, `src/ground.js`, `src/terrain.js`, `src/debugOverlay.js`. **67/67 unique tests green.**
- Files written: `test/e2e/{harness.mjs, README.md, boot.test.mjs, core.test.mjs, features.test.mjs, boot-clean.test.mjs, teleport-nan.test.mjs, debug-flag.test.mjs}`, `test/data/airspacesSchema.test.js`, `data/airspaces.schema.json`, `src/main.js`, `src/ground.js`, `src/terrain.js`, `src/debugOverlay.js`, `.gitignore`; docs: this block, `state_TODO.md` (frontmatter + §0h + §2 + §3), `README.md` Testing section, `docs/30-spec/SPEC.md` §6 row 73.
- Open follow-ups added to `state_TODO.md`: §0h still-open rows (paused-freeze frame gate; `_drawMapUnderlay` NaN-center note).
